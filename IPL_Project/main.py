...
for _, row in batsman_stats_df.iterrows():
    ...
    if row['powerplay_balls_faced'] < 50:
        ...
        batsman_stats[row['batter']] = {
            'powerplay_run_rate': max(row['run_rate']-0.3,0.1),
            'powerplay_avg_runs_per_ball': max(row['avg_runs_per_ball']-0.3,0.1),
            'powerplay_std_dev_runs_per_ball': row['std_dev_runs_per_ball'],
            'powerplay_dismissal_rate_per_100': row['dismissal_rate_per_100']+2,
